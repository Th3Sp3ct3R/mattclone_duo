...
export const api = {
  ...
  engine: {
    ...
    // Broker the ARMVM/RedFinger live-control handshake. `uuid` is generated by the
    // caller (LiveControl) and must match the value BgsSdk bakes into its clientToken.
    async connectLiveControl(deviceId, { uuid, ...quality } = {}) {
      const res = await http().post(`/v1/engine/devices/${deviceId}/live/connect`, { uuid, ...quality });
      return res.data;
    },
    async liveHeartbeat(deviceId, type = 1) {
      const res = await http().post(`/v1/engine/devices/${deviceId}/live/heartbeat`, { type });
      return res.data;
    },
