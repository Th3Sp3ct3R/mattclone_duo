...
test('blocks another YouTube account on the same device', () => {
  const conflict = findAccountDevicePlatformConflict(
    [
      {
        _id: 'account-1',
...
test('allows Instagram, TikTok, and YouTube to coexist on the same device', () => {
  const accounts = [
    { _id: 'account-1', platform: 'instagram', assignedDeviceId: 'device-1', retiredAt: null },
    { _id: 'account-2', platform: 'tiktok', assignedDeviceId: 'device-1', retiredAt: null }
  ];

  expect(
    findAccountDevicePlatformConflict(accounts, {
      platform: 'youtube',
      assignedDeviceId: 'device-1',
      accountId: 'account-3'
    })
