...
import { toDeviceDisplay } from '@julio/api/utils/provider-display';
...
function publicDevice(device = {}) {
  ...
  // Merge display-only fields. Raw `provider` is kept for admin/debug and
  // queued jobs; UI consumes `providerDisplay` / `tier` / `displayLabel`.
  const display = toDeviceDisplay(plain);
  return {
    ...plain,
    providerDisplay: display.providerDisplay,
    tier: display.tier,
    tierDisplay: display.tierDisplay,
    displayLabel: display.displayLabel,
    deviceModel: display.deviceModel,
    fleetGroup: display.fleetGroup
  };
}
...
// --- Live control (Path C): broker the ARMVM/RedFinger handshake -------------
// The browser cannot mint these tokens itself (the /image/* API is session-gated),
// so the server runs startCheck -> start -> connect -> connectTokenShared using the
// captured DuoPlus session, and hands the client exactly what BgsSdk needs:
// appId + serverToken (+ padCode/hosts). See docs/duoplus-endpoints-captured.md §3b.

function pickResultInfo(connectResponse) {
  const data = connectResponse?.data || {};
  return data.token?.resultInfo || data.resultInfo || null;
}

function pickServerToken(sharedResponse, imageId) {
  const data = sharedResponse?.data || {};
  if (typeof data.serverToken === 'string' && data.serverToken) return data.serverToken;
  if (typeof data.server_token === 'string' && data.server_token) return data.server_token;
  if (Array.isArray(data.serverTokens) && data.serverTokens.length) {
    const first = data.serverTokens[0];
    return typeof first === 'string' ? first : first?.serverToken || first?.token || '';
  }
  const list = data.list || data.tokens || data.items;
  if (Array.isArray(list) && list.length) {
    const match = list.find((e) => (e.image_id || e.imageId) === imageId) || list[0];
    return match?.serverToken || match?.server_token || match?.token || '';
  }
  return typeof data.token === 'string' ? data.token : '';
}

export async function connectDuoPlusLiveControl(req, res) {
  try {
    requireAdmin(req);
    await ensureDb();
    const device = await findDeviceOr404(req.params.id);
    if (device.provider !== 'duoplus') {
      return res.status(400).json({ code: 'BAD_REQUEST', message: 'Live control is only available for DuoPlus devices' });
    }
    // The uuid is generated by the browser (it is baked into BgsSdk.getClientToken),
    // so the same value must key connectTokenShared. Require it from the client.
    const uuid = String(req.body?.uuid || '').trim();
    if (!uuid) {
      return res.status(400).json({ code: 'BAD_REQUEST', message: 'uuid is required (generate it client-side)' });
    }
    const imageId = String(device.providerDeviceId || '').trim();
    if (!imageId) {
      return res.status(400).json({ code: 'BAD_REQUEST', message: 'Device has no providerDeviceId' });
    }
    ...
    try {
      ...
      logger.warn('DuoPlus live control unavailable: no captured session. Re-login the duoplus-chrome profile.', {
        message: err.message
      });
      return res.json({ ok: false, reason: 'no-session', message: err.message });
    }

    try {
      const startCheck = await client.startCheck(imageId).catch(() => ({ data: {} }));
      const started = await client.start(imageId);
      const connected = await client.connect(imageId);
      const shared = await client.connectTokenShared([imageId], uuid);

      const resultInfo = pickResultInfo(connected);
      const serverToken = pickServerToken(shared, imageId);
      if (!resultInfo || !serverToken) {
        logger.error('DuoPlus live control: incomplete handshake', {
          hasResultInfo: Boolean(resultInfo),
          hasServerToken: Boolean(serverToken)
        });
        return res.json({ ok: false, reason: 'incomplete-handshake', message: 'connect/connectTokenShared returned no token' });
      }

      const appId = resultInfo?.merchantInfo?.appkey || '';
      const padCode = resultInfo?.padCode || resultInfo?.padList?.[0]?.padCode || '';
      const domain = String(resultInfo?.domain || 'https://oversea.armvm.com');
      // The media-negotiation host observed live is the "-platform" sibling of the domain.
      const connectPhoneUrl = `${domain.replace('oversea.', 'oversea-platform.')}/sdk/instance/cloud-phone-connect`;
      // onlineTime = remaining lease seconds; fall back to a full day if the lease
      // response omits it so BgsSdk's required-param check passes.
      const onlineTime = Number(started?.data?.duration_seconds || started?.data?.onlineTime || 86400);

      const quality = resolveFocusQuality({
        width: req.body?.w ?? device.runtime?.screenWidth,
        height: req.body?.h ?? device.runtime?.screenHeight,
        bitrate: req.body?.bitrate,
        fps: req.body?.fps,
        clarity: req.body?.clarity
      });

      // Never ship the merchant appSecret to the browser — server-side only.
      const safeResultInfo = resultInfo ? { ...resultInfo, merchantInfo: { appkey: appId } } : null;

      return res.json({
        ok: true,
        imageId,
        uuid,
        sdkUrl: '/BgsSdk.min.1.54.0.dp.js',
        appId,
        padCode,
        serverToken,
        onlineTime,
        ...
        connectPhoneUrl,
        quality,
        routes: startCheck?.data?.route_list || [],
        resultInfo: safeResultInfo
      });
    } catch (err) {
      if (err?.code === 'DUOPLUS_SESSION_EXPIRED') {
        logger.warn('DuoPlus session expired during live-control handshake. Re-capture it.');
        return res.json({ ok: false, reason: 'expired', message: 'DuoPlus session expired; re-capture it.' });
      }
      throw err;
    }
  } catch (err) {
    logger.error('Engine DuoPlus live control connect failed', err);
    return sendError(res, err, 'Internal error');
  }
}

export async function heartbeatDuoPlusLiveControl(req, res) {
  try {
    requireAdmin(req);
    await ensureDb();
    const device = await findDeviceOr404(req.params.id);
    ...
    const imageId = String(device.providerDeviceId || '').trim();
    if (!imageId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'Device has no providerDeviceId' });
    // 1 = active control (foreground), 3 = background keepalive.
    const type = Number(req.body?.type ?? 1) === 3 ? 3 : 1;
    ...
    try {
      await client.heartbeat(imageId, type);
      return res.json({ ok: true, imageId, type });
    } catch (err) {
      if (err?.code === 'DUOPLUS_SESSION_EXPIRED') {
        return res.json({ ok: false, reason: 'expired', message: 'DuoPlus session expired; re-capture it.' });
      }
    ...
    logger.error('Engine DuoPlus live control heartbeat failed', err);
    return sendError(res, err, 'Internal error');
  }
}
